# Terrain-Aware AGM Distance Calculator — Final Calibrated Build
...
st.title("Terrain-Aware AGM Distance Calculator (Calibrated to Google Earth)")
...
# Tuned constants
SPACING_M = 10.0            # ← coarser resampling (drops micro-zigzags)
SIMPLIFY_TOLERANCE_M = 20.0 # ← heavier smoothing
SMOOTH_WINDOW = 7
...
def agm_sort_key(pair):
    n = pair[0]; d = ''.join(filter(str.isdigit, n)); s = ''.join(filter(str.isalpha, n))
    return (int(d) if d else -1, s)

def parse_kml_kmz(f):
    if f.name.endswith(".kmz"):
        with zipfile.ZipFile(f) as zf:
            kml = next((n for n in zf.namelist() if n.endswith(".kml")), None)
            if not kml: return [], []
            with zf.open(kml) as fh: data = fh.read()
    else: data = f.read()

    root = ET.fromstring(data); agms, cls = [], []
    for folder in root.findall(".//{*}Folder"):
        name = (folder.find("{*}name").text or "").strip().upper() if folder.find("{*}name") is not None else ""
        if name == "AGMS":
            for pm in folder.findall(".//{*}Placemark"):
                nm, cr = pm.find("{*}name"), pm.find(".//{*}coordinates")
            ...
            for pm in folder.findall(".//{*}Placemark"):
                ...
                pts=[]
                for p in cr.text.strip().split():
                    try: lon, lat, *_ = map(float, p.split(",")); pts.append((lon, lat))
                    except: pass
...
def decode_rgb(r,g,b): R,G,B=int(r),int(g),int(b); return -10000+((R*256*256)+(G*256)+B)*0.1
...
class TerrainCache:
    ...
    def get(self,z,x,y):
        ...
        r=requests.get(TERRAIN_TILE_URL.format(z=z,x=x,y=y),
...
def smooth(v,k):
    if k<=1:return v
    arr=np.asarray(v,float); ker=np.ones(k)/k
...
def sample(ls,s1,s2,sp):
    xs,ys,cum=build_arrays(ls); s_lo,s_hi=sorted((s1,s2))
...
if u:
    ...
    parts_m=[tf_line(p,xf_fwd).simplify(SIMPLIFY_TOLERANCE_M,False) for p in parts]
    ...
    for i in range(len(agms)-1):
        ...
        for j in range(len(pts)-1):
            ...
            dz=elev[j+1]-elev[j]; dist_m+=math.hypot(dxy,dz)
