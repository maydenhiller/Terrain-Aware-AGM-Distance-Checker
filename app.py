# Terrain-Aware AGM Distance Calculator — Geodesic 3D (Fixed Overflow + Working Parser)
...
st.title("Terrain-Aware AGM Distance Calculator (Geodesic 3D, Stable Build)")
...
with st.sidebar:
    ...
    st.caption("zoom 17, spacing 1 m ≈ best accuracy")

# ───────────────────────────── PARSER (working version)
def agm_sort_key(name_geom):
    name = name_geom[0]
    d = ''.join(filter(str.isdigit, name))
    base = int(d) if d else -1
    sfx = ''.join(filter(str.isalpha, name)).upper()
    return (base, sfx)
...
def parse_kml_kmz(uploaded):
    if uploaded.name.endswith(".kmz"):
        with zipfile.ZipFile(uploaded) as zf:
            ...
            if not kml_file: return [], []
            with zf.open(kml_file) as f: data = f.read()
    else:
        data = uploaded.read()
        ...
        if folder_name == "agms":
            ...
            for pm in folder.findall("kml:Placemark", ns):
                ...
                for pair in cr.text.strip().split():
                    ...
                    pts.append((lon, lat))
                if len(pts) >= 2: parts.append(LineString(pts))
...
def decode_terrain_rgb(r,g,b):
    R, G, B = int(r), int(g), int(b)          # <-- overflow-safe cast
    return -10000.0 + ((R*256*256)+(G*256)+B)*0.1

class TerrainCache:
    def __init__(self, token, zoom=17): self.t=token; self.z=zoom; self.c={}
    def get(self,z,x,y):
        k=(z,x,y)
    ...
    def elev(self,lon,lat):
        try:
            ...
            x0,y0=int(xp),int(yp)
            dx,dy=xp-x0,yp-y0
            x0=max(0,min(255,x0));y0=max(0,min(255,y0))
            x1=min(x0+1,255);y1=min(y0+1,255)
...
def smooth(vals,k):
    if k<=1: return vals
    kern=np.ones(int(k))/float(k)
    return np.convolve(np.asarray(vals,float),kern,"same").tolist()
...
def sample(ls,s1,s2,sp):
    ...
    out=[]
    for ds in steps: out.append(interp(xs,ys,cum,s_lo+ds))
    X,Y=zip(*out); return np.array(X),np.array(Y)

def choose_part(parts_m,p1_ll,p2_ll,xf_ll_to_m,max_off):
    p1m,p2m=tf_pt(p1_ll,xf_ll_to_m),tf_pt(p2_ll,xf_ll_to_m)
    best=None
...
if up:
    ...
    for p in parts_ll:
        pm=tf_line(p,xf_fwd)
        if simplify_tolerance_m>0: pm=pm.simplify(simplify_tolerance_m)
    ...
    for i in range(len(agms)-1):
        ...
        if part_idx is None: skipped+=1; continue
        part=parts_m[part_idx]
        Xs,Ys=sample(part,s1,s2,spacing_m)
        if Xs.size<2: skipped+=1; continue
