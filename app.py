# Terrain-Aware AGM Distance Calculator — Final Parser Fix (Google Earth Matched)
...
st.title("Terrain-Aware AGM Distance Calculator (Parser Fixed + GE Calibrated)")
...
# ───────────── KML/KMZ parser (namespace-agnostic)
def agm_sort_key(p):
...
def parse_kml_kmz(upload):
    if upload.name.endswith(".kmz"):
        with zipfile.ZipFile(upload) as z:
            kml = next((n for n in z.namelist() if n.endswith(".kml")), None)
            if not kml: return [], []
            data = z.read(kml)
    else:
        data = upload.read()
    ...
    # ignore XML namespaces completely
    for folder in root.iter():
        if folder.tag.lower().endswith("folder"):
            name_el = next((c for c in folder if c.tag.lower().endswith("name")), None)
            if not name_el or not name_el.text:
                continue
            fname = name_el.text.strip().upper()
            if fname == "AGMS":
                for pm in folder.iter():
                    if pm.tag.lower().endswith("placemark"):
                        nm = next((c for c in pm if c.tag.lower().endswith("name")), None)
                        coord_el = next((c for c in pm.iter() if c.tag.lower().endswith("coordinates")), None)
                ...
                for pm in folder.iter():
                    if pm.tag.lower().endswith("placemark"):
                        ...
                        if coord_el is None or not coord_el.text:
                            continue
                        pts = []
                        for pair in coord_el.text.strip().split():
                            try:
