# app.py
# Terrain-Aware AGM Distance Calculator — FAST (Bulk Elevation Version)
# Implements tile-based Mapbox fetch (1 call per tile, vectorized interpolation)
...
class TerrainCache:
    ...
    def fetch_tile(self, z: int, x: int, y: int):
        ...
        if r.status_code == 403:
            st.error("❌ Mapbox 403 Forbidden — token lacks tileset access.")
            st.stop()
    ...
    def elevations_bulk(self, lons: np.ndarray, lats: np.ndarray) -> np.ndarray:
        ...
        for i, (xx, yy) in enumerate(zip(x_tile, y_tile)):
            k = (z, int(xx), int(yy))
            tile_groups.setdefault(k, []).append(i)
        for k, idxs in tile_groups.items():
            arr = self.fetch_tile(*k)
            if arr is None:
                continue
            idxs = np.array(idxs)
...
# Helpers
# ------------------------------------------------------------
def smooth_moving_average(a, window_m, spacing_m):
    if len(a) < 3: return a
...
def parse_kml_kmz(f):
    if f.name.lower().endswith(".kmz"):
        with zipfile.ZipFile(f) as zf:
            ...
            data = zf.read(kml_name) if kml_name else b""
    else:
        data = f.read()
    ...
    for folder in root.findall(".//Folder"):
        ...
        if not name_el or not name_el.text:
            continue
        if name_el.text.strip() == "AGMs":
            for pm in folder.findall(".//Placemark"):
                nm_el = pm.find("name")
                if not nm_el or not nm_el.text:
                    continue
                nm = nm_el.text.strip()
        ...
        if name_el.text.strip() == "CENTERLINE":
            for pm in folder.findall(".//Placemark"):
                coords_el = pm.find(".//LineString/coordinates")
                if coords_el is None:
                    continue
                pts = [tuple(map(float, t.split(",")[:2])) for t in coords_el.text.split()]
                if len(pts) >= 2:
                    clines.append(LineString(pts))
